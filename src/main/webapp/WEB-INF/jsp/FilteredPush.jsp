...
<div>
<h2>필터푸시</h2>
deviceToken 뽑는 쿼리 : (결과는 USER테이블에서 device_tocken,id, device_type 이, device_type여야함 ) <br> ex) select device_token, id, device_type from USER where nickname = '광조동방'<br>
<textarea id="query" form="pushForm" name="query" rows="10" cols="100" >${query}</textarea><br>
<form id="pushForm" action="push-filtered" method="post" onsubmit="return confirm('정말로 푸시 할꺼임???????? 5분정도 걸림');" >
  <label class='flabel'>channel_ids(콤마로구분):</label><input type="text" name="channelIds" size='35'><br>
  <label class='flabel'>message:</label><textarea name="msg" form="pushForm" rows="10" cols="100"></textarea><br>
  <input class='btn' type="submit" onclick="onSubmit()" value="필터푸시 실행하기">
</form>
</div>
<br>
...
function loadDoc() {
 ...
 var x;
   ...
   xhttp.open("GET", '/push-filtered?message=' + msg, true);
    xhttp.send();
  } else {
